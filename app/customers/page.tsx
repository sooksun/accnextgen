...
export default function CustomersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        resetForm();
        fetchCustomers();
        if (selectedCustomer?.id === editingId) fetchCustomerDetail(editingId);
        toast.success(editingId ? "บันทึกการแก้ไขแล้ว" : "เพิ่มลูกค้าแล้ว");
      } else {
  ...
  async function handleDelete(id: string) {
    const ok = await confirm({ title: "ลบลูกค้า", message: "ต้องการลบลูกค้านี้หรือไม่?", variant: "danger", confirmLabel: "ลบ" });
    if (!ok) return;
    try {
  ...
  async function handleDeleteUnit(unitId: string) {
    ...
    const ok = await confirm({ title: "ลบหน่วยงาน", message: "ต้องการลบหน่วยงานนี้หรือไม่?", variant: "danger", confirmLabel: "ลบ" });
    if (!ok) return;
    try {
