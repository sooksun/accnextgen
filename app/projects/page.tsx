...
export default function ProjectsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        closeEdit();
        fetchProjects();
        if (detailProject && editId === detailProject.id) fetchProjectDetail(editId);
        toast.success(editId ? "บันทึกการแก้ไขแล้ว" : "สร้างโครงการแล้ว");
      } else {
  ...
  async function handleDelete(id: string) {
    const ok = await confirm({
      title: "ลบโครงการ",
      message: "คุณต้องการลบโครงการนี้หรือไม่? การดำเนินการนี้ไม่สามารถย้อนกลับได้",
      variant: "danger",
      confirmLabel: "ลบ",
    });
    if (!ok) return;
          ...
          <CardContent className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block">วันเริ่ม</label>
                  <ThaiDatePicker
              ...
              <div>
                ...
                {form.milestones.map((ms, idx) => (
                  ...
                  >
                    ...
                    <div className="col-span-3">
                      <ThaiDatePicker
                        value={ms.dueDate || null}
                        onChange={(v) =>
                          updateMilestone(idx, "dueDate", v ?? "")
                        }
                      />
                    </div>
