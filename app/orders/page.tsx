...
export default function OrdersPage() {
  ...
  async function handleStatusChange(orderId: string, newStatus: string) {
    ...
    const ok = await confirm({ message: `ต้องการเปลี่ยนสถานะเป็น "${statusLabel}" หรือไม่?` });
    if (!ok) return;
  ...
  async function handleDelete(orderId: string) {
    const ok = await confirm({
      title: "ลบออเดอร์",
      message: "ต้องการลบออเดอร์นี้หรือไม่? การลบจะไม่สามารถกู้คืนได้",
      variant: "danger",
      confirmLabel: "ลบ",
    });
    if (!ok) return;
  ...
  const hasDocumentOfType = useCallback((type: DocType) => {
    return selectedOrder?.documents.some(doc => doc.type === type);
  }, [selectedOrder]);

  async function handleGenerateDocument(docType: DocType, orderId: string) {
    const ok = await confirm({
      message: `ต้องการสร้าง ${docTypeLabels[docType]} สำหรับออเดอร์นี้หรือไม่?`,
    });
    if (!ok) return;
    ...
    try {
      const res = await fetch("/api/documents", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId, type: docType }),
      });

      if (res.ok) {
        const newDoc = await res.json();
        toast.success(`สร้าง ${docTypeLabels[docType]} สำเร็จ`);
        fetchOrderDetail(orderId); // Refresh order details to show new document
        window.open(`/api/documents/${newDoc.id}/pdf`, "_blank"); // Open PDF
      } else {
        const err = await res.json();
  ...
  const renderDocumentButtons = useCallback(() => {
    if (!selectedOrder) return null;

    const buttons = [];

    // DRAFT: Generate Quotation
    if (selectedOrder.status === "DRAFT" && !hasDocumentOfType("QUOTATION")) {
      buttons.push(
        <Button
          key="quotation"
          size="sm"
          variant="outline"
          onClick={() => handleGenerateDocument("QUOTATION", selectedOrder.id)}
        >
          <FileText className="h-3 w-3 mr-1" /> สร้างใบเสนอราคา
        </Button>
      );
    }

    // INVOICED: Generate Delivery Note, Invoice
    if (selectedOrder.status === "INVOICED") {
      if (!hasDocumentOfType("DELIVERY_NOTE")) {
        buttons.push(
          <Button
            key="deliveryNote"
            size="sm"
            variant="outline"
            onClick={() => handleGenerateDocument("DELIVERY_NOTE", selectedOrder.id)}
          >
            <Truck className="h-3 w-3 mr-1" /> สร้างใบส่งสินค้า
          </Button>
        );
      }
      if (!hasDocumentOfType("INVOICE")) {
        buttons.push(
          <Button
            key="invoice"
            size="sm"
            variant="outline"
            onClick={() => handleGenerateDocument("INVOICE", selectedOrder.id)}
          >
            <FileText className="h-3 w-3 mr-1" /> สร้างใบแจ้งหนี้
          </Button>
        );
      }
    }

    // PAID: Generate Tax Invoice/Receipt
    if (selectedOrder.status === "PAID" && !hasDocumentOfType("TAX_INVOICE_RECEIPT")) {
      buttons.push(
        <Button
          key="taxInvoiceReceipt"
          size="sm"
          variant="default"
          onClick={() => handleGenerateDocument("TAX_INVOICE_RECEIPT", selectedOrder.id)}
        >
          <FileText className="h-3 w-3 mr-1" /> สร้างใบกำกับภาษี/ใบเสร็จ
        </Button>
      );
    }

    return buttons;
  }, [selectedOrder, hasDocumentOfType, handleGenerateDocument]);
