...
export default function ProductsPage() {
  ...
  async function addGroup() {
    ...
    try {
      ...
      else { const e = await res.json(); alert(e.error || "เกิดข้อผิดพลาด"); }
    } catch (err) { console.error(err); }
  }
  ...
  async function deleteGroup(id: string, name: string) {
    if (!confirm(`ลบหมวดหมู่ "${name}"?\nสินค้าที่อยู่ในหมวดนี้จะถูกย้ายเป็น "ไม่มีหมวดหมู่"`)) return;
    try {
      const res = await fetch(`/api/product-groups?id=${id}`, { method: "DELETE" });
      if (res.ok) { fetchGroups(); fetchProducts(); }
    } catch (err) { console.error(err); }
  }
                ...
                products.map((product) => {
                  ...
                  <TableRow key={product.id} className={product.isActive ? "" : "opacity-50"}>
                    ...
                    <TableCell>
                      ...
                      {product.scrapedAt && <div className="text-[10px] text-slate-300">ดึงข้อมูล: {new Date(product.scrapedAt).toLocaleDateString("th-TH")}</div>}
                    </TableCell>
                    <TableCell>
                      {product.group ? (
