...
interface DocDetail {
  ...
  grandTotal: string;
  note: string | null;
  customer: { name: string; taxId: string | null; address: string | null; email: string | null; phone: string | null };
  order: { code: string } | null;
  project: { code: string; title: string } | null;
  items: DocDetailItem[];
  withholdingTax: { rate: string; baseAmount: string; taxAmount: string; certificateNo: string | null } | null;
}
...
export default function DocumentsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(err.error || "เกิดข้อผิดพลาดในการสร้างเอกสาร");
      }
    } catch (err) {
      console.error(err);
  ...
  async function openDetail(id: string) {
    ...
    try {
      ...
      if (res.ok) setDetailDoc(await res.json());
      else toast.error("โหลดรายละเอียดไม่สำเร็จ");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700 mb-1 block">วันที่ออกเอกสาร *</label>
                  <ThaiDatePicker
                    value={form.issueDate}
                    onChange={(v) => setForm(prev => ({ ...prev, issueDate: v ?? "" }))}
                  />
                </div>
              </div>
                  ...
                  <TableRow key={doc.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button size="icon" variant="ghost" title="ดูรายละเอียด" onClick={() => openDetail(doc.id)}>
      ...
      {/* รายละเอียดเอกสาร (เมื่อคลิกดู) */}
      {selectedDocId && (
        <Card className="border-blue-200 shadow-lg">
          <CardHeader className="bg-blue-50 border-b border-blue-100 py-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg text-blue-900 flex items-center gap-2">
                {detailDoc ? docTypeLabels[detailDoc.type]?.icon : null}
                รายละเอียดเอกสาร {detailDoc ? detailDoc.number : selectedDocId}
              </CardTitle>
              <div className="flex gap-1">
                <Button size="sm" variant="outline" onClick={() => selectedDocId && handlePrintPdf(selectedDocId)} disabled={!detailDoc}>
            ...
            {detailLoading || !detailDoc ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-sm font-medium text-slate-500 mb-2">ลูกค้า</h4>
                    <div className="bg-slate-50 rounded-lg p-4 space-y-1">
                      <div className="font-semibold">{detailDoc.customer.name}</div>
                      {detailDoc.customer.taxId && <div className="text-sm text-slate-500">เลขผู้เสียภาษี: {detailDoc.customer.taxId}</div>}
                      {detailDoc.customer.address && <div className="text-sm text-slate-500">{detailDoc.customer.address}</div>}
                      {detailDoc.customer.email && <div className="text-sm text-slate-500">{detailDoc.customer.email}</div>}
                      {detailDoc.customer.phone && <div className="text-sm text-slate-500">{detailDoc.customer.phone}</div>}
                    </div>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-slate-500 mb-2">ข้อมูลเอกสาร</h4>
                    <div className="bg-slate-50 rounded-lg p-4 space-y-2">
                      <div className="flex justify-between"><span className="text-slate-500">วันที่ออกเอกสาร</span><span className="font-medium">{formatDateShort(detailDoc.issueDate)}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">อัตรา VAT</span><span className="font-mono">{Number(detailDoc.vatRate)}%</span></div>
                      {detailDoc.order && <div className="flex justify-between"><span className="text-slate-500">อ้างอิงออเดอร์</span><Badge variant="outline">{detailDoc.order.code}</Badge></div>}
                      {detailDoc.project && <div className="flex justify-between"><span className="text-slate-500">อ้างอิงโครงการ</span><Badge variant="outline">{detailDoc.project.code}</Badge></div>}
                      {detailDoc.note && <div className="pt-2 border-t"><span className="text-slate-500">หมายเหตุ: </span><span className="text-sm">{detailDoc.note}</span></div>}
                    </div>
                  </div>
                </div>

                <div>
                  <h4 className="text-sm font-medium text-slate-500 mb-2">รายการ ({detailDoc.items.length} รายการ)</h4>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-right w-28">รวม</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {detailDoc.items.map((item, idx) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell><div className="font-medium">{item.name}</div>{item.description && <div className="text-xs text-slate-400">{item.description}</div>}</TableCell>
                  ...
                  <div className="flex justify-end mt-3">
                    <div className="w-72 space-y-1">
                      <div className="flex justify-between text-sm"><span className="text-slate-500">ฐานภาษี</span><span className="font-mono">{formatNumber(detailDoc.subTotal)}</span></div>
                      <div className="flex justify-between text-sm"><span className="text-slate-500">VAT {Number(detailDoc.vatRate)}%</span><span className="font-mono">{formatNumber(detailDoc.vatAmount)}</span></div>
                      {detailDoc.withholdingTax && (
                        <div className="flex justify-between text-sm"><span className="text-slate-500">หัก ณ ที่จ่าย {Number(detailDoc.withholdingTax.rate)}%</span><span className="font-mono">-{formatNumber(detailDoc.withholdingTax.taxAmount)}</span></div>
                      )}
                      <div className="flex justify-between text-lg font-bold border-t pt-1"><span>ยอดรวม</span><span className="font-mono text-blue-700">{formatNumber(detailDoc.grandTotal)}</span></div>
                    </div>
                  </div>
                </div>
              </div>
