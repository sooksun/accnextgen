...
export function renderInvoice(data: InvoiceData): string {
  ...
  const content = `
    ...
    <div class="footer">
      <p style="font-size:12px; color:#666;">กรุณาชำระเงินภายใน 30 วัน นับจากวันที่ในใบแจ้งหนี้</p>
      <div class="signatures">
        <div class="signature-box">
          <div class="signature-line">ผู้ออกใบแจ้งหนี้</div>
        </div>
        <div class="signature-box">
          <div class="signature-line">ผู้รับใบแจ้งหนี้</div>
        </div>
      </div>
    </div>
  `;

  return baseTemplate(content, `ใบแจ้งหนี้ ${data.number}`);
}
