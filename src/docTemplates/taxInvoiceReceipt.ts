...
interface TaxInvoiceReceiptData {
  number: string;
...
export function renderTaxInvoiceReceipt(data: TaxInvoiceReceiptData): string {
  ...
  const whtSection = data.wht
    ? `
    <tr>
      <td>ภาษีหัก ณ ที่จ่าย ${data.wht.rate}%</td>
      <td class="text-right" style="color:red">- ${pdfNumber(data.wht.taxAmount)}</td>
    </tr>
    <tr class="grand-total">
      <td><strong>ยอดรับสุทธิ</strong></td>
      <td class="text-right"><strong>${pdfNumber(Number(data.grandTotal) - Number(data.wht.taxAmount))}</strong></td>
    </tr>`
    : "";
  ...
  const content = `
    <div class="header">
      ...
      <div class="doc-info">
        <div class="doc-type" style="color:#15803d">ใบกำกับภาษี/ใบเสร็จรับเงิน</div>
  ...
  return baseTemplate(content, `ใบกำกับภาษี/ใบเสร็จรับเงิน ${data.number}`);
}
